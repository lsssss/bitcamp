...
public class Test21_8 {
	...
	public static void main(String[] args) {
		...
		int j = Integer.parseInt(console.readLine("몇명 입력 하시겠습니까 ? : "));
		prof[] pf = new prof[j];
		for(int i = 0; i < j ; i++) {
			pf[i] = new prof();
		}
		...
		for( int k = 0; k < pf.length; k++) {
			yn = console.readLine("입력 하시 겠습니까? ex) y or n : ");
			if(yn.equals("y")) {
				profinit(pf[k], console);
				save = console.readLine("저장 하시 겠습니까 ? ex) y or n : ");
				if(save.equals("n")) {
					pf[k].name = null;
					pf[k].email = null;
					pf[k].phonenum = null;
					...
					System.out.println("y나 n을 입력 하세요");
				}
			}else if(yn.equals("n")) {
				k=pf.length;
